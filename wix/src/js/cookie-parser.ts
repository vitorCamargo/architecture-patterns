...
import { isWix, isSameDomain } from './utils';
...
function getConsentPolicyHeader(
  ...
  const raw = Object.keys(mappingFromCookie).reduce(
    (ret: { [name: string]: number }, key) => {
      ...
      //@ts-ignore
      ret[key] = policy[value] ? 1 : 0;
      return ret;
    },
    {},
  );

  const str = encodeURIComponent(JSON.stringify(raw));
  return { [COOKIE_NAME]: str };
}

function getCookieData() {
  const regex = `[;\\s ]?${COOKIE_NAME}=([\\S]+(?:;?))`;
  const cookieData = document.cookie.match(regex);
  let match = cookieData ? cookieData.pop() : void 0;
